...
public:
    ...
    vector<Token> lexer()
    {
        ...
        do
        {
            ...
            if (this->current == '\0' && position.index == 0)
            {
                // file empty, quit.
                break;
            }
            ...
            if (this->current == '\n' || this->current == ';')
            {
                ...
                {
                    tokens.push_back(Token(NEW_LINE));
                    previous_line = true;
                }
                continue;
            }
            ...
            else
            {
                ...
                case '{':
                    add(tokens, Token(LEFT_BRACKETS));
                    break;
                case '}':
                    add(tokens, Token(RIGHT_BRACKETS));
                    break;
    ...
    Token id()
    {
        if (isalpha(this->current))
        {
            ...
            {
                back();
                return Token(FUNCTION_DEFITION);
            }
            else if (idstr == "import")
            {
                back();
                return Token(IMPORT_STATEMENT);
            }
            else if (idstr == "val")
            {
                back();
                return Token(VARIABLE_DEFINITION);
            }
            else
